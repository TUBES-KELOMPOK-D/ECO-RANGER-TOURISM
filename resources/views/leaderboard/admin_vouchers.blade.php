    ...
    <div class="bg-slate-900 shadow-lg py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Kelola Voucher</h1>
                    <p class="text-slate-400 font-medium">Manajemen reward penukaran poin untuk pengguna.</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 space-y-8">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h2 class="text-lg font-bold text-slate-900">Daftar Voucher Reward</h2>
            ...
            <div id="createForm" class="hidden p-6 border-b border-slate-100 bg-slate-50/30">
                <form action="{{ route('leaderboard.admin.vouchers.store') }}" method="POST" class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" placeholder="contoh: Voucher Wisata Rp 100rb" required class="w-full rounded-xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm p-3 bg-white">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Poin Reward</label>
                        <input type="number" name="poin_required" placeholder="500" required class="w-full rounded-xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm p-3 bg-white">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deskripsi</label>
                        <input type="text" name="description" placeholder="Syarat dan ketentuan penukaran..." required class="w-full rounded-xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm p-3 bg-white">
                    </div>
                ...
                <table class="w-full text-left text-sm text-slate-600">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-500 border-b border-slate-100 font-bold text-xs uppercase tracking-widest">
                            ...
                            <th class="px-6 py-4 text-center">Poin Reward</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-5 text-center font-black text-emerald-600">{{ number_format($voucher->poin_required) }} pts</td>
                            <td class="px-6 py-5 text-slate-500 text-xs truncate max-w-xs">{{ $voucher->description }}</td>
                            <td class="px-6 py-5 text-right flex justify-end gap-2">
                                <button onclick="document.getElementById('editVoucher-{{ $voucher->id }}').classList.toggle('hidden')" class="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors">
                        ...
                        <!-- EDIT VOUCHER FORM -->
                        <tr id="editVoucher-{{ $voucher->id }}" class="hidden bg-slate-50/80">
                            <td colspan="4" class="px-6 py-6 border-x border-emerald-100">
                                <form action="{{ route('leaderboard.admin.vouchers.update', $voucher) }}" method="POST" class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                                    @csrf
                                    @method('PUT')
                                    <div class="sm:col-span-2">
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Nama Voucher</label>
                                        <input type="text" name="name" value="{{ $voucher->name }}" required class="w-full rounded-xl border-slate-200 text-sm p-3 bg-white">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Poin Reward</label>
                                        <input type="number" name="poin_required" value="{{ $voucher->poin_required }}" required class="w-full rounded-xl border-slate-200 text-sm p-3 bg-white">
                                    </div>
                                    <div class="sm:col-span-2">
                                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Deskripsi</label>
                                        <input type="text" name="description" value="{{ $voucher->description }}" required class="w-full rounded-xl border-slate-200 text-sm p-3 bg-white">
                                    </div>
                                    <div class="flex items-end gap-2">
                                        ...
                                        <button type="button" onclick="document.getElementById('editVoucher-{{ $voucher->id }}').classList.add('hidden')" class="px-6 py-3 rounded-xl border border-slate-200 text-sm font-bold text-slate-500 hover:bg-white transition-all">Batal</button>
                                    </div>
                                </form>
                            </td>
                        </tr>
