            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 class="text-4xl font-black tracking-tight leading-tight mb-2">
                        Aturan <span class="text-emerald-300">Poin</span>
                    </h1>
                    <p class="text-emerald-100/80 text-sm font-medium">Konfigurasi reward poin untuk setiap aktivitas pengguna.</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 space-y-8">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                <h2 class="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Daftar Aktivitas</h2>
            ...
            <div id="createForm" class="hidden p-8 border-b border-slate-100 bg-slate-50/50">
                <form action="{{ route('leaderboard.admin.point_rules.store') }}" method="POST" class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Nama Aktivitas</label>
                        <input type="text" name="action_name" placeholder="contoh: Lapor Isu Lingkungan" required class="w-full rounded-2xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm p-4 bg-white font-bold">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Poin Reward</label>
                        <input type="number" name="points_reward" placeholder="10" required class="w-full rounded-2xl border-slate-200 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm p-4 bg-white font-bold">
                    </div>
                    <div class="flex items-end gap-3 sm:col-span-3">
                        <button type="submit" class="flex-1 rounded-2xl bg-slate-900 px-8 py-4 text-xs font-black text-white hover:bg-slate-800 transition-all shadow-xl uppercase tracking-widest">Simpan Aturan</button>
                ...
                <table class="w-full text-left text-sm text-slate-600">
                    <thead>
                        <tr class="bg-slate-50/50 text-slate-400 border-b border-slate-100 font-black text-[10px] uppercase tracking-[0.2em]">
                            <th class="px-8 py-5">Aktivitas & Identitas</th>
                            <th class="px-8 py-5 text-center">Reward Poin</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    <div class="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 border border-emerald-100 group-hover:scale-110 transition-transform duration-300">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v10"/><path d="M18 8l-6 6-6-6"/></svg>
                                    </div>
                                    <div>
                                        <div class="font-black text-slate-900 text-base tracking-tight">{{ $rule->action_name }}</div>
                                        <div class="text-[10px] text-slate-400 font-mono mt-0.5 uppercase tracking-wider">{{ $rule->action_key }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6 text-center">
                                <span class="inline-flex items-center gap-1.5 font-black text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100">
                                    +{{ number_format($rule->points_reward, 0, ',', '.') }} PTS
                                </span>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <div class="flex justify-end gap-2">
                                    <button onclick="document.getElementById('editRule-{{ $rule->id }}').classList.toggle('hidden')" class="p-3 text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all">
                        ...
                        <!-- EDIT RULE FORM -->
                        <tr id="editRule-{{ $rule->id }}" class="hidden">
                            <td colspan="3" class="px-8 py-8 bg-emerald-50/50">
                                <form action="{{ route('leaderboard.admin.point_rules.update', $rule) }}" method="POST" class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                                    ...
                                    <div class="sm:col-span-2">
                                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Nama Aktivitas</label>
                                        <input type="text" name="action_name" value="{{ $rule->action_name }}" required class="w-full rounded-2xl border-slate-200 text-sm p-4 bg-white font-bold">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3">Reward Poin</label>
                                        <input type="number" name="points_reward" value="{{ $rule->points_reward }}" required class="w-full rounded-2xl border-slate-200 text-sm p-4 bg-white font-bold">
                                    </div>
                                    <div class="flex items-end gap-3 sm:col-span-3">
                                        <button type="submit" class="flex-1 rounded-2xl bg-emerald-600 px-8 py-4 text-xs font-black text-white hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20 uppercase tracking-widest">Update Aturan</button>
                                        <button type="button" onclick="document.getElementById('editRule-{{ $rule->id }}').classList.add('hidden')" class="px-8 py-4 rounded-2xl border border-slate-200 text-xs font-black text-slate-500 hover:bg-white transition-all uppercase tracking-widest">Batal</button>
                                    </div>
                                </form>
                            </td>
                        </tr>
