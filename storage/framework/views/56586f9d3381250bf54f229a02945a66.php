<nav class="sticky top-0 z-[100] bg-white/80 backdrop-blur-xl border-b border-slate-200/60 font-sans">
        ...
        <div class="flex justify-between h-20 items-center">
            ...
            <div class="hidden md:flex items-center gap-1 lg:gap-4">
                ...
                <?php if(!request()->is('login') && !request()->is('register')): ?>
                    <a href="/" class="px-4 py-2 rounded-xl text-sm font-bold transition-all decoration-transparent <?php echo e(request()->is('/') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'); ?>">Beranda</a>
                    <a href="/aksi" class="px-4 py-2 rounded-xl text-sm font-bold transition-all decoration-transparent <?php echo e(request()->is('aksi') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'); ?>">Aksi</a>
                    <a href="/peringkat" class="px-4 py-2 rounded-xl text-sm font-bold transition-all decoration-transparent <?php echo e(request()->is('peringkat') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'); ?>">Peringkat</a>
                    <a href="/reports" class="px-4 py-2 rounded-xl text-sm font-bold transition-all decoration-transparent <?php echo e(request()->is('reports') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'); ?>">Laporan</a>
                    <a href="/akademi" class="px-4 py-2 rounded-xl text-sm font-bold transition-all decoration-transparent <?php echo e(request()->is('akademi') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-50'); ?>">Akademi</a>                    
                    ...
                    <div class="relative group ml-4">
                        <button class="w-10 h-10 rounded-full overflow-hidden border-2 transition-all bg-emerald-100 text-emerald-700 border-emerald-500 hover:scale-110 focus:outline-none">
                            <?php if(auth()->user()->photo): ?>
                                <img src="<?php echo e(asset('storage/' . auth()->user()->photo)); ?>" alt="Foto Profil" class="h-full w-full object-cover" />
                            <?php else: ?>
                                <span class="inline-flex h-full w-full items-center justify-center font-bold"><?php echo e(strtoupper(substr(auth()->user()->name, 0, 2))); ?></span>
                            <?php endif; ?>
                        </button>
                        <div class="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                            ...
                            <form action="<?php echo e(route('logout')); ?>" method="POST" class="border-t border-slate-100">
                                <?php echo csrf_field(); ?>
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-white border-b border-slate-100 shadow-xl absolute w-full left-0 transition-all">
        <div class="px-4 pt-2 pb-6 space-y-1">
            <?php if(!request()->is('login') && !request()->is('register')): ?>
                <a href="/" class="flex items-center gap-4 w-full p-4 rounded-2xl transition-all font-bold decoration-transparent <?php echo e(request()->is('/') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'); ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                    Beranda
                </a>
                <a href="/aksi" class="flex items-center gap-4 w-full p-4 rounded-2xl transition-all font-bold decoration-transparent <?php echo e(request()->is('aksi') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'); ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                    Aksi
                </a>
                <a href="/peringkat" class="flex items-center gap-4 w-full p-4 rounded-2xl transition-all font-bold decoration-transparent <?php echo e(request()->is('peringkat') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'); ?>">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>
                    Peringkat
                </a>
                <a href="/akademi" class="flex items-center gap-4 w-full p-4 rounded-2xl transition-all font-bold decoration-transparent <?php echo e(request()->is('akademi') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-slate-50'); ?>">
                    ...
                    <a href="/profile" class="flex items-center gap-3 w-full p-4 rounded-2xl font-bold bg-slate-50 text-slate-700 hover:bg-slate-100 transition-all">
                        <div class="w-8 h-8 rounded-lg overflow-hidden bg-emerald-100">
                            <?php if(auth()->user()->photo): ?>
                                <img src="<?php echo e(asset('storage/' . auth()->user()->photo)); ?>" alt="Foto Profil" class="h-full w-full object-cover" />
                            <?php else: ?>
                                <div class="flex h-full w-full items-center justify-center text-xs font-bold text-emerald-700"><?php echo e(strtoupper(substr(auth()->user()->name, 0, 2))); ?></div>
                            <?php endif; ?>
                        </div>
                        Profil Saya
                    </a>
...
</script><?php /**PATH D:\Project\ECO-RANGER-TOURISM\resources\views/components/navbar.blade.php ENDPATH**/ ?>