<?php $__env->startSection('title', 'Pengaturan Profil - GreenTour'); ?>
...
<div class="mt-6 mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
    <div class="rounded-3xl bg-white p-8 shadow-soft border border-slate-200">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="<?php echo e(route('profile.index')); ?>" class="rounded-full border border-slate-200 bg-slate-50 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-100">Kembali ke Profil</a>
        </div>
        ...
        <div class="mt-8 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            <div class="space-y-8">
                <form action="<?php echo e(route('profile.settings.update')); ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
                    <?php echo csrf_field(); ?>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="space-y-3">
                            <label class="text-xs font-semibold uppercase tracking-[0.3em] text-slate-400">Nama Lengkap</label>
                            <input type="text" name="name" value="<?php echo e(old('name', $user->name)); ?>" class="w-full rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4 text-slate-900 shadow-sm outline-none focus:border-toscagreen focus:ring-2 focus:ring-toscagreen/10" placeholder="Andi Saputra" />
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="rounded-3xl border border-emerald-100 bg-slate-50 px-4 py-4 text-slate-900">
                                <div class="flex items-center justify-between gap-4">
                                    <span><?php echo e($user->eco_level); ?></span>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-4">
                            <div id="profilePreview" class="relative h-28 w-28 overflow-hidden rounded-[2rem] border-2 border-toscagreen bg-emerald-50 text-4xl font-black uppercase text-toscagreen shadow-md">
                                <img id="previewImage" src="<?php echo e($user->photo ? asset('storage/'.$user->photo) : ''); ?>" alt="Foto Profil" class="h-full w-full object-cover <?php echo e($user->photo ? '' : 'hidden'); ?>" />
                                <div id="previewInitials" class="flex h-full w-full items-center justify-center <?php echo e($user->photo ? 'hidden' : ''); ?>"><?php echo e(strtoupper(substr($user->name, 0, 2))); ?></div>
                            </div>
                            <div>
                ...
                <?php if($user->photo): ?>
                    <form action="<?php echo e(route('profile.photo.delete')); ?>" method="POST" class="mt-4">
                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Project\ECO-RANGER-TOURISM\eco-ranger-tourism\resources\views/Profile/settings.blade.php ENDPATH**/ ?>